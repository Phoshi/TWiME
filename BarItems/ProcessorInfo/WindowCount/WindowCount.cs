...
namespace ProcessorInfo {
    public class WindowCount : IPluginBarItem {
        ...
        public Bitmap Draw(string argument) {
            int numWindows = (from screen in _parent.Parent.screens select screen.windows).SelectMany(window => window).Count();
            string outString = "{0}{1}{2}".With(_prepend, numWindows, _append);
